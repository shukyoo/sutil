...
class Query
{
     ...
     * @return $this
     */
    public function andWhereGroup($where)
    {
        if (!is_null($where)) {
            if ($this->_where) {
                $this->_where .= ' AND ('. $this->_where($where) .')';
     ...
     * @return $this
     */
    public function orWhereGroup($where)
    {
        if (!is_null($where)) {
            $this->_where .= ' OR ('. $this->_where($where) .')';
        }
    ...
    /**
     * if empty where clause, it will return false
     * @param null $where
     * @return bool
     */
    public function exists($where = null)
    {
        ...
        $sql = $this->_grammar->exists($this->_table, $this->_where);
        return (bool)$this->_connection->fetchOne($sql, $this->getBind());
    }
    ...
    {
        return $this->_connection->fetchAllIndexed($this->getSql(), $this->getBind());
    }
